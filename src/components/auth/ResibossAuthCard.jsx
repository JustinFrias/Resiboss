...
export const ResibossAuthCard = ({ initialMode = 'signin' }) => {
  ...
  const isGoogleDisabled = isLoading || isGoogleLoading || !isOnline;
        ...
        >
          {isGoogleLoading ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              <Loader2 size={18} className="animate-spin" style={{ color: isLight ? '#0284c7' : '#00f2fe' }} />
              <span>{isFil ? 'Kumokonekta sa Google...' : 'Connecting to Google...'}</span>
            </div>
          ) : (
            <>
        ...
        >
          {isLoading ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              ...
              <span>
                {mode === 'signin'
                  ? (isFil ? 'Pumapasok...' : 'Signing in...')
                  : (isFil ? 'Gumagawa ng account...' : 'Creating account...')}
              </span>
            </div>
          ) : (
            <>
              <span>
                {mode === 'signin'
                  ? (isFil ? 'Mag-sign in' : 'Sign In')
                  : (isFil ? 'Gumawa ng Account' : 'Create Account')}
              </span>
              <ArrowRight size={17} />
            </>
          )}
